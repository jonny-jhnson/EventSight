...
def parse_evtx_incremental(
    file_path: str,
    event_ids: set[int],
    since_timestamp: Optional[datetime] = None,
    max_events: int = 500
) -> tuple[list[WindowsEvent], Optional[datetime]]:
    """
    Parse EVTX file or live channel incrementally, filtering at parse time.

    Designed for continuous analysis mode - only returns events that:
    1. Match the specified event IDs
    2. Have timestamps after since_timestamp (if provided)

    Supports two modes:
    - Log file mode: If path ends with .evtx and file exists, uses /lf flag
    - Channel mode: Otherwise treats input as a channel name (e.g., "Security")
      Channel mode works without admin privileges for live monitoring.

    Args:
        file_path: Path to EVTX file OR channel name (e.g., "Security", "Application")
        event_ids: Set of Event IDs to include (others are skipped)
        since_timestamp: Only include events after this timestamp
        max_events: Maximum events to return per call

    Returns:
        Tuple of (events list, last_timestamp seen)
        last_timestamp can be used for the next incremental call
    """
    import subprocess
    import shutil
    ...
    # Determine if this is a log file or channel name
    is_log_file = path.suffix.lower() == '.evtx' and path.exists()

    # For log files, verify the file exists
    if path.suffix.lower() == '.evtx' and not path.exists():
    ...
    if is_log_file:
        # Log file mode - use /lf flag
        cmd = [wevtutil, 'qe', str(path), '/lf', '/f:xml']
    else:
        # Channel mode - no /lf flag (works without admin for live logs)
        cmd = [wevtutil, 'qe', file_path, '/f:xml']
    ...
    try:
        result = subprocess.run(
            ...
            capture_output=True,
            text=True,
            timeout=120,  # 2 minute timeout for incremental
            encoding='utf-8',
            errors='replace'
        )
        ...
        for match in event_pattern.finditer(xml_content):
            xml_str = match.group()

            # Quick Event ID extraction before full parse
            ...
            if ts_match:
                try:
                    ...
                    if last_timestamp is None or event_ts > last_timestamp:
                        last_timestamp = event_ts
                except (ValueError, AttributeError):
                    pass

            # Full parse only for matching events
            try:
                event = _parse_xml_record(xml_str)
                if event:
                    events.append(event)
