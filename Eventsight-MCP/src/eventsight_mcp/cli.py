...
def print_help():
    ...
    commands = [
        ("evaluate <file> [options]", "Analyze EVTX file. Options: --all, --batch N, --filter-file FILE, --event-ids IDS", "evaluate Security.evtx --all"),
    ...
    while True:
        try:
            ...
            if command == "exit" or command == "quit":
                ...
                if not args:
                    console.print("[red]Usage: evaluate <file_path> [--all] [--batch N][/red]")
                    continue

                # Check for --all flag
                parse_all = "--all" in args
                args_clean = args.replace("--all", "")
                ...
                if parse_all:
                    console.print("[cyan]Parsing entire EVTX file(s) (this may take a while for large files)...[/cyan]")
                if batch_size:
...
@click.option('--all', 'parse_all', is_flag=True, help='Parse entire EVTX file(s) instead of default limit')
...
def evaluate(file_paths, parse_all, max_events, batch_size, no_smart_filter, no_filter, event_ids, filter_file):
    """Analyze EVTX file(s) for suspicious activity.
    ...
    For large files, use --all to parse everything. By default, smart filtering
    will identify security-relevant Event IDs first to reduce API usage.

    You can also specify exact Event IDs with --event-ids "1,4688,4624"
    ...
    try:
        ...
        if parse_all:
            print("Parsing entire EVTX file(s) (this may take a while for large files)...")
